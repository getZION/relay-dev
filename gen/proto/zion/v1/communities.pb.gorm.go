...
type CommunityORM struct {
	...
	OwnerAlias      string `gorm:"not null"`
	OwnerPubkey     string `gorm:"not null"`
...
func (m *Community) ToORM(ctx context.Context) (CommunityORM, error) {
	...
	to.OwnerPubkey = m.OwnerPubkey
	to.OwnerAlias = m.OwnerAlias
...
func DefaultApplyFieldMaskCommunity(ctx context.Context, patchee *Community, patcher *Community, updateMask *field_mask.FieldMask, prefix string, db *gorm.DB) (*Community, error) {
	...
	for _, f := range updateMask.Paths {
		...
		if f == prefix+"OwnerPubkey" {
			patchee.OwnerPubkey = patcher.OwnerPubkey
			continue
		}
		if f == prefix+"OwnerAlias" {
			patchee.OwnerAlias = patcher.OwnerAlias
			continue
		}
